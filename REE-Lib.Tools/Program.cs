...
internal sealed class Program
{
    ...
    {
        Console.WriteLine("ReeLib.Tools.exe <COMMAND> [ARGS...]");
    ...
    private static void GenerateEFXTemplates(string outputFolder)
    {
        ...
        basedir = basedir.Substring(0, basedir.IndexOf($"{Path.DirectorySeparatorChar}REE-Lib.Tools"));

        var files = new SyntaxTree [] {
            CSharpSyntaxTree.ParseText(File.ReadAllText(Path.Combine(basedir, "REE-Lib/OtherFiles/EfxFile.cs")))
        }.Concat(Directory.GetFiles(Path.Combine(basedir, "REE-Lib/OtherFiles/EFX")).Select(fn => CSharpSyntaxTree.ParseText(File.ReadAllText(fn))));
        ...
        var autogeneratedNotice = "// autogenerated using REE Lib: https://github.com/kagenocookie/RE-Engine-Lib";
                    ...
                    default:
                        {
                            ...
                            local uint errorUniqueId = ReadUInt(FTell()+4);
                            Printf("Struct ID does not match any known structs, template stopping.\nItem Type:%i\nuniqueId:%i\nPosition:%i\n",errorItemType,errorUniqueId,FTell());
